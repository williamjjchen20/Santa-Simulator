...
def resetApp(app, level, totalGifts, resetGame):
    ...
    app.level = level ### Credit to my roommate Nathan Sefrin (not in 15112) for giving me the level system idea, all code for it is done by me
    ...
    app.santaStartScreenX = 0
    app.santaStartScreenY = 100
...
def resetBoard(app, numHouses, numObstacles):
    ...
    app.houseDist = 3 if app.resetCounter == 0 else 1
    state = generateHouses(app)
    if state == 'crashed':
        print('here')
        resetBoard(app, numHouses, numObstacles)
...
def onStep(app):
    ...
    if app.gameStart:
        app.santaFrame += 1
        app.santaStartScreenX += 10
        if app.santaStartScreenX > app.width:
            app.santaStartScreenX = -10
            app.santaStartScreenY += 100
            if app.santaStartScreenY > app.height:
                app.santaStartScreenY = 100
    ...
    if not app.paused:
        if app.selectedGift != None:
            ...
            if (app.boardWidth > mouseX and mouseX > 0) and (app.boardHeight > mouseY and mouseY > 0):
                ...
                ###Checks each house for the gift drop
                for house in app.houses:
...
def drawStartScreen(app):
    ...
    drawImage(santaImage, app.santaStartScreenX, app.santaStartScreenY, width=100, height=100)
...
def generateHouses(app):
    ...
    for i in range(app.numHouses):
        state = generateHouse(app, i)
        if state == 'crashed':
            return state

def generateHouse(app, i):
    if app.resetCounter > 10:
        return 'crashed'
...
def drawHouses(app):
    ...
    for house in app.houses:
        ...
        drawPolygon(houseX, houseY + app.cellHeight-1, houseX+app.cellWidth/2, houseY+app.cellHeight+5, houseX+app.cellWidth, houseY+app.cellHeight-1, fill='gray', opacity=30)
...
def drawInventory(app):
    drawRect(app.boardWidth, 0, app.inventoryWidth, app.height, fill= 'aliceBlue' if app.screen=='default-screen' else'blanchedalmond')
    drawLine(app.boardWidth, 0, app.boardWidth, app.height, lineWidth=2)
    drawImage(app.giftImage, app.boardWidth+app.inventoryWidth/2, 20, align='center', width=80, height=60, opacity=30)
    drawLabel('Gifts', app.boardWidth+app.inventoryWidth/2, 20, align='center', bold=True, size=20, fill='navy' if app.screen=='default-screen' else 'maroon')
...
def drawExtraDefault(app):
    drawRect(0, app.boardHeight, app.boardWidth-1, app.bottomHeight, fill='lightCyan')
    drawLabel(f'Level: {app.level}', app.boardWidth/6, app.boardHeight + app.bottomHeight/2, bold=True, fill='midnightBlue', size=20)
    drawLabel(f"Press 'g' to make gifts!", app.boardWidth/2, app.boardHeight + app.bottomHeight/2, italic=True, size=16)
    drawTimer(app)
    drawImage(app.wreathImage, 30, app.boardHeight+app.bottomHeight/2, align='center', width=60, height=60)
...
def drawTimer(app):
    drawLabel(f'Time: {math.floor(app.gameTimer)} s', 5*app.boardWidth/6, app.boardHeight + app.bottomHeight/2, bold=True, fill='midnightBlue', size=20)
...
def fillGaps(app): ### The transparent images didn't crop that well, so this is just covering some of the weird pixels 
...
def drawRecipeBook(app):
    ...
    for recipe in app.recipes:
        ...
        drawImage(gift, app.width/7+firstGap-i, app.height/8+10+60*i, width=app.giftImageWidth, height=app.giftImageHeight, align='right', opacity=70)
        ...
        for material in app.recipes[recipe]:
            if material != 'tool':
                ...
                drawLabel(f'{count} x ', app.width/10+firstGap+materialGap*j-i, app.height/8+10+recipeGap*i, size=15)
                drawImage(image, app.width/10+firstGap+materialGap*j + 7-i, app.height/8+10+recipeGap*i, width=app.materialImageWidth, height=app.materialImageHeight, align='left', opacity=70)
            else:
                tool = app.recipes[recipe][material]
                image = app.toolImageDict[tool]
                drawImage(image, app.width/2+3*app.width/8-10+i, app.height/8+10+recipeGap*i, width=app.materialImageWidth, height=app.materialImageHeight, align='right', opacity=70)
            j+=1
        i+=1 
